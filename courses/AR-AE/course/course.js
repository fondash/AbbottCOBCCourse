...
lnx.nav = {
    ...
    disableFwdNav: function(){
        
        if(!this.isLinearNav) return false;            
        if(lnx.scormApi.getIsCourseComplete()) return false;
        if(this.navIndex > this.farIndex) return false;
        if(this.isScreenComplete(this.currNavId)) return false;
    ...
    navigate: function( navId, viaEdits, isShift ) {
        ...
        if(navId === "next" && this.insertVirtual(prevType, navId)){
            ...
            if(this.screenLocked || specialCaseVirtualScreen()){
                lnx.view.disableFwdButton();
            }
        	return;
        }

        function specialCaseVirtualScreen(){
            // special case for virtual screens that grey out the forward button on final screen of set   
            var screen = lnx.view.getCurrentScreen();
            if(screen.customGreyOutFwdButton && screen.customGreyOutFwdButton()){
                return self.navIndex >= self.farIndex;
            } else {
                return false;   
            }
...
lnx.sliderIcons = {
    ...
    init : function( node, screenElm, frameElm, origNavId){
        ...
        this.singleScreen = node.getAttribute("subType") === "single";
        this.screen = this.singleScreen ? 2 : 0;
    ...
    insertVirtualScreen: function(dir, showFinalReverse){        
        ...
        if(dir === "next"){
                if(this.screen < this.numScreens){
                    this.screen++;
                    this.updateVirtualScreen(dir);
                    if(this.singleScreen && !this.complete){
                        lnx.nav.disableFwdNav();
                    }
                    return true;
                }else{
            ...
            if(this.screen <= 1 || (this.singleScreen && this.screen <=3)){
                return false;
            }else{
                this.screen--;
                this.updateVirtualScreen(dir);
    ...
    customGreyOutFwdButton: function(){
        if(this.complete){
            return false;
        }
        if(this.singleScreen){
