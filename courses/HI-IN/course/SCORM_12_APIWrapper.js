// note modifications made by learnex
// some global vars prefixed with adl_
// timer functions added from SCOFunctions.js, licence included below
...
var adl_debug = true;  // set this to false to turn debugging off

var adl_output = window.console; // output can be set to any object that has a log(string) function
...
var adl_initialized = false;

// local variable definitions
var adl_apiHandle = null;
...
function doLMSInitialize()
{
   if (adl_initialized) return "true";
...
function doLMSFinish()
{
   ...
   {
	   // added from SCOFunctions.js
	   var compTime = computeTime()
	   doLMSSetValue( "cmi.core.session_time", compTime );
	   doLMSCommit();
...
function doLMSGetValue(name)
{
   ...
   else if (! adl_initialized && ! doLMSInitialize())
   {
...
function getAPIHandle()
{
   if (adl_apiHandle == null)
   {
      adl_apiHandle = getAPI();
   }

   return adl_apiHandle;
}
...
// functionality from SCOFunctions.js

function startTimer()
{
   adl_startDate = new Date().getTime();
}
...
function computeTime()
{
   if ( adl_startDate != 0 )
   {
      ...
      var elapsedSeconds = ( (currentDate - adl_startDate) / 1000 );
